...
describe("OAuth login", () => {
  it("redirects delegated accounts before asking for a password", async () => {
    ...
    window.history.replaceState({}, "", `/?request_uri=${encodeURIComponent(requestUri)}`);
    const fetchMock = vi.fn(async (input: RequestInfo | URL) => {
      const url = String(input);
      if (url.startsWith("/oauth/authorize?")) return json({ client_name: "Test client" });
    ...
    render(
      <MemoryRouter initialEntries={[`/app/oauth/login?request_uri=${encodeURIComponent(requestUri)}`]}>
        <Routes>
          <Route path="/app/oauth/login" element={<OAuthLoginPage />} />
          <Route path="/app/oauth/delegation" element={<h1>Delegation login</h1>} />
        </Routes>
      </MemoryRouter>,
    );

    await userEvent.setup().type(screen.getByLabelText("Handle or email"), "alice.example.com");

    expect(await screen.findByRole("heading", { name: "Delegation login" })).toBeInTheDocument();
  ...
  it("preserves Tranquil's pending verification data", async () => {
    ...
    const fetchMock = vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
      ...
      if (url === "/oauth/sso/providers") return json({ providers: [] });
      if (url.startsWith("/oauth/security-status?")) return json({ isDelegated: false });
    ...
    await user.type(screen.getByLabelText("Handle or email"), "alice.example.com");
    await user.type(screen.getByLabelText("Password"), "password123");
    await user.click(screen.getByRole("button", { name: "Sign in" }));

    expect(await screen.findByRole("link", { name: "Verify your account" })).toHaveAttribute(
      "href",
  ...
  it("follows Tranquil's delegated-account redirect response", async () => {
    ...
    const user = userEvent.setup();
    await user.type(screen.getByLabelText("Handle or email"), "alice.example.com");
    await user.type(screen.getByLabelText("Password"), "not-used-for-delegation");
    await user.click(screen.getByRole("button", { name: "Sign in" }));
