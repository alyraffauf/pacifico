...
export function OAuthCodePage() {
  ...
  async function submit(event: React.FormEvent) {
    ...
    try {
      const result = await readJson<{ redirect_uri?: string }>(
        await fetch(getOAuthCodeEndpoint(mode), {
          method: "POST",
          headers: {
  ...
  return (
    <AuthLayout
      title={
        isTotp ? "Two-factor authentication" : "Enter your verification code"
      }
      description={
        isTotp
          ? "Enter the six-digit code from your authenticator, or an eight-character backup code."
          : `We sent a six-digit code through ${channel}.`
      }
    >
      <form className="grid gap-4" onSubmit={submit}>
        {error ? <Alert tone="error">{error}</Alert> : null}
        <Field
          label={isTotp ? "Authenticator or backup code" : "Verification code"}
        >
          <Input
            value={code}
        ...
        <div className="flex justify-end gap-2">
          ...
          <Button disabled={submitting || code.trim().length < 6}>
