...
describe("OAuth flow routing", () => {
  it("accepts Tranquil's delegated-account redirect response", () => {
    expect(getAuthorizationDestination(
      { redirect: "/app/oauth/delegation?request_uri=abc&delegated_did=did%3Aplc%3Aalice" },
      "urn:request:abc",
    )?.startsWith("/app/oauth/delegation?")).toBe(true);
  });
