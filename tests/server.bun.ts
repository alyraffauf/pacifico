...
beforeAll(async () => {
  ...
  await Bun.write(join(distributionDirectory, "assets/app-abc123.js"), "export {};");
});
...
describe("Pacifico server", () => {
  it("applies the enforced security policy to HTML", async () => {
    const response = await handler()(new Request("https://pds.example.com/app/settings"));
  ...
  it("uses the configured origin in OAuth client metadata", async () => {
    const response = await handler()(new Request("https://pds.example.com/oauth-client-metadata.json"));
  ...
  it("rejects a mismatched host", async () => {
    const response = await handler()(new Request("https://attacker.example/oauth-client-metadata.json"));
    expect(response.status).toBe(421);
  });

  it("only serves files with GET and HEAD", async () => {
    const response = await handler()(new Request("https://pds.example.com/assets/app-abc123.js", {
      method: "POST",
    }));
  ...
  it("rejects malformed encoded paths", async () => {
    const response = await handler()(new Request("https://pds.example.com/%E0%A4%A"));
    expect(response.status).toBe(400);
  });

  it("allows direct pod health probes without relaxing public host validation", async () => {
    const response = await handler()(new Request("http://127.0.0.1:3000/healthz"));
    expect(response.status).toBe(200);
    expect(await response.text()).toBe("ok");
    expect(response.headers.get("content-security-policy")).not.toBeNull();

    const publicResponse = await handler()(new Request("http://127.0.0.1:3000/app/settings"));
    expect(publicResponse.status).toBe(421);

    const writeResponse = await handler()(new Request("http://127.0.0.1:3000/healthz", {
      method: "POST",
    }));
